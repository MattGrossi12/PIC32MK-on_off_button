...
void analog_process_sample(void)
{
    ...
    if ((g_adc_voltage >= GREEN_VOLTAGE_MIN) &&
        (g_adc_voltage <= GREEN_VOLTAGE_MAX))
    {
        g_voltage_zone = 1u;
        RLED_OFF();
        GLED_ON();
    ...
    else if ((g_adc_voltage >= YELLOW_VOLTAGE_MIN) &&
             (g_adc_voltage <= YELLOW_VOLTAGE_MAX))
    {
        g_voltage_zone = 2u;
        RLED_OFF();
        GLED_OFF();
    ...
    else if ((g_adc_voltage >= RED_VOLTAGE_MIN) &&
             (g_adc_voltage <= RED_VOLTAGE_MAX))
    {
        g_voltage_zone = 3u;
        RLED_ON();
        GLED_OFF();
